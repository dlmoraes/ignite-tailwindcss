...
const fileItem = tv({
  slots: {
    ...
    icon: 'rounded-full border-4 border-violet-100 bg-violet-200 text-violet-400 dark:border-zinc-700 dark:bg-zinc-800 dark:text-zinc-500',
    deleteButton: '',
  },
  variants: {
    state: {
      ...
      error: {
        container:
          'bg-error-25 border-error-300 dark:bg-error-500/5 dark:border-error-300/30',
    ...
    <div className={container()}>
      ...
      {state === 'error' ? (
        <div className="flex flex-1 flex-col items-start gap-1">
          ...
          <button
            type="button"
            className="text-sm font-semibold text-red-700 hover:text-red-900 dark:text-error-400 dark:hover:text-error-300"
          >
            Try again
          </button>
        </div>
